document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("http://192.168.1.215:3000/posts", {
    method: "GET",
    headers: {
      Authorization: `${token}`,
    },
  ...
  function createMediaElement(mediaUrl) {
    ...
    console.log("Formatted URL:", formattedUrl); // Log the formatted URL for debugging
    ...
    if (formattedUrl.match(/\.(jpeg|jpg|gif|png|heic|HEIC)$/i)) {
      ...
      console.error("Unsupported media type:", formattedUrl); // Log unsupported media type for debugging
    }
